package com.chriscorp.celer.nostr
...
import com.chriscorp.celer.services.SeenMessageStore
import com.chriscorp.celer.ui.ChatState
import com.chriscorp.celer.ui.MeshDelegateHandler
import com.chriscorp.celer.ui.PrivateChatManager
...
class NostrDirectMessageHandler(
    ...
    private val dataManager: com.chriscorp.celer.ui.DataManager
) {
    ...
    fun onGiftWrap(giftWrap: NostrEvent, geohash: String, identity: NostrIdentity) {
        scope.launch(Dispatchers.Default) {
            try {
                ...
                if (packet.type != com.chriscorp.celer.protocol.MessageType.NOISE_ENCRYPTED.value) return@launch

                val noisePayload = com.chriscorp.celer.model.NoisePayload.decode(packet.payload) ?: return@launch
                val messageTimestamp = Date(giftWrap.createdAt * 1000L)
                val convKey = "nostr_${senderPubkey.take(16)}"
                repo.putNostrKeyMapping(convKey, senderPubkey)
                com.chriscorp.celer.nostr.GeohashAliasRegistry.put(convKey, senderPubkey)
    ...
    private suspend fun processNoisePayload(
        payload: com.chriscorp.celer.model.NoisePayload,
        convKey: String,
        ...
        when (payload.type) {
            com.chriscorp.celer.model.NoisePayloadType.PRIVATE_MESSAGE -> {
                val pm = com.chriscorp.celer.model.PrivateMessagePacket.decode(payload.data) ?: return
            ...
            com.chriscorp.celer.model.NoisePayloadType.FILE_TRANSFER -> {
                ...
                val file = com.chriscorp.celer.model.BitchatFilePacket.decode(payload.data)
                if (file != null) {
                    val uniqueMsgId = java.util.UUID.randomUUID().toString().uppercase()
                    val savedPath = com.chriscorp.celer.features.file.FileUtils.saveIncomingFile(application, file)
                    val message = BitchatMessage(
                        id = uniqueMsgId,
                        sender = senderNickname,
                        content = savedPath,
