package com.chriscorp.celer.mesh

import android.content.Context
import android.util.Log
import com.chriscorp.celer.crypto.EncryptionService
import com.chriscorp.celer.model.BitchatMessage
import com.chriscorp.celer.protocol.MessagePadding
import com.chriscorp.celer.model.RoutedPacket
import com.chriscorp.celer.model.IdentityAnnouncement
import com.chriscorp.celer.protocol.BitchatPacket
import com.chriscorp.celer.protocol.MessageType
import com.chriscorp.celer.protocol.SpecialRecipients
import com.chriscorp.celer.model.RequestSyncPacket
import com.chriscorp.celer.sync.GossipSyncManager
import com.chriscorp.celer.util.toHexString
...
class BluetoothMeshService(private val context: Context) {
        ...
        gossipSyncManager = GossipSyncManager(
            ...
            configProvider = object : GossipSyncManager.ConfigProvider {
                override fun seenCapacity(): Int = try {
                    com.chriscorp.celer.ui.debug.DebugPreferenceManager.getSeenPacketCapacity(500)
                } catch (_: Exception) { 500 }

                override fun gcsMaxBytes(): Int = try {
                    com.chriscorp.celer.ui.debug.DebugPreferenceManager.getGcsMaxFilterBytes(400)
                } catch (_: Exception) { 400 }

                override fun gcsTargetFpr(): Double = try {
                    com.chriscorp.celer.ui.debug.DebugPreferenceManager.getGcsFprPercent(1.0) / 100.0
    ...
    private fun setupDelegates() {
        ...
        messageHandler.delegate = object : MessageHandlerDelegate {
            ...
            override fun updatePeerIDBinding(newPeerID: String, nickname: String,
                ...
                try {
                    com.chriscorp.celer.favorites.FavoritesPersistenceService.shared.findNostrPubkey(publicKey)?.let { npub ->
                        com.chriscorp.celer.favorites.FavoritesPersistenceService.shared.updateNostrPublicKeyForPeerID(newPeerID, npub)
                    }
                } catch (_: Exception) { }
        ...
        connectionManager.delegate = object : BluetoothConnectionManagerDelegate {
            ...
            override fun onDeviceConnected(device: android.bluetooth.BluetoothDevice) {
                ...
                try {
                    ...
                    com.chriscorp.celer.ui.debug.DebugSettingsManager.getInstance()
     ...
     */
    fun sendFileBroadcast(file: com.chriscorp.celer.model.BitchatFilePacket) {
        try {
     ...
     */
    fun sendFilePrivate(recipientPeerID: String, file: com.chriscorp.celer.model.BitchatFilePacket) {
        try {
            ...
            serviceScope.launch {
                ...
                if (encryptionService.hasEstablishedSession(recipientPeerID)) {
                    try {
                        ...
                        val noisePayload = com.chriscorp.celer.model.NoisePayload(
                            type = com.chriscorp.celer.model.NoisePayloadType.FILE_TRANSFER,
                            data = filePayload
                        )
        ...
        serviceScope.launch {
            ...
            if (encryptionService.hasEstablishedSession(recipientPeerID)) {
                try {
                    ...
                    val privateMessage = com.chriscorp.celer.model.PrivateMessagePacket(
                    ...
                    val messagePayload = com.chriscorp.celer.model.NoisePayload(
                        type = com.chriscorp.celer.model.NoisePayloadType.PRIVATE_MESSAGE,
                        data = tlvData
                    )
    ...
    fun sendReadReceipt(messageID: String, recipientPeerID: String, readerNickname: String) {
        serviceScope.launch {
            ...
            val geo = runCatching { com.chriscorp.celer.services.MessageRouter.tryGetInstance() }.getOrNull()
            val isGeoAlias = try {
                val map = com.chriscorp.celer.nostr.GeohashAliasRegistry.snapshot()
                map.containsKey(recipientPeerID)
            } catch (_: Exception) { false }
            if (isGeoAlias && geo != null) {
                geo.sendReadReceipt(com.chriscorp.celer.model.ReadReceipt(messageID), recipientPeerID)
                return@launch
            }
            
            try {
                // Create read receipt payload using NoisePayloadType exactly like iOS
                val readReceiptPayload = com.chriscorp.celer.model.NoisePayload(
                    type = com.chriscorp.celer.model.NoisePayloadType.READ_RECEIPT,
     ...
     */
    fun getSessionState(peerID: String): com.chriscorp.celer.noise.NoiseSession.NoiseSessionState {
