...
import com.chriscorp.celer.ui.debug.DebugSettingsManager
import com.chriscorp.celer.ui.debug.DebugScanResult
...
class BluetoothGattClientManager(
    ...
    fun start(): Boolean {
        ...
        try {
            if (!com.chriscorp.celer.ui.debug.DebugSettingsManager.getInstance().gattClientEnabled.value) {
    ...
    fun onScanStateChanged(shouldScan: Boolean) {
        val enabled = try { com.chriscorp.celer.ui.debug.DebugSettingsManager.getInstance().gattClientEnabled.value } catch (_: Exception) { true }
