...
import com.chriscorp.celer.model.BitchatFilePacket
...
class MediaSendingManager(
    ...
    fun sendFileNote(toPeerIDOrNull: String?, channelOrNull: String?, filePath: String) {
        try {
            ...
            val mimeType = try { 
                com.chriscorp.celer.features.file.FileUtils.getMimeTypeFromExtension(file.name) 
            } catch (_: Exception) { 
    ...
    private fun sendPrivateFile(
        ...
        messageManager.updateMessageDeliveryStatus(
            msg.id,
            com.chriscorp.celer.model.DeliveryStatus.PartiallyDelivered(0, 100)
        )
     ...
     */
    fun handleTransferProgressEvent(evt: com.chriscorp.celer.mesh.TransferProgressEvent) {
        val msgId = synchronized(transferMessageMap) { transferMessageMap[evt.transferId] }
        if (msgId != null) {
            if (evt.completed) {
                messageManager.updateMessageDeliveryStatus(
                    msgId,
                    com.chriscorp.celer.model.DeliveryStatus.Delivered(to = "mesh", at = java.util.Date())
                )
                synchronized(transferMessageMap) {
                ...
                messageManager.updateMessageDeliveryStatus(
                    msgId,
                    com.chriscorp.celer.model.DeliveryStatus.PartiallyDelivered(evt.sent, evt.total)
                )
            }
        }
    }
