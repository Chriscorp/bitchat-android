...
import com.chriscorp.celer.core.ui.utils.singleOrTripleClickable
...
fun TorStatusIcon(
    ...
    val torStatus by com.chriscorp.celer.net.TorManager.statusFlow.collectAsState()
    
    if (torStatus.mode != com.chriscorp.celer.net.TorMode.OFF) {
        val cableColor = when {
...
fun PeerCounter(
    ...
    isConnected: Boolean,
    selectedLocationChannel: com.chriscorp.celer.geohash.ChannelID?,
    ...
    Row(
        ...
        Icon(
            ...
            contentDescription = when (selectedLocationChannel) {
                is com.chriscorp.celer.geohash.ChannelID.Location -> "Geohash participants"
...
private fun PrivateChatHeader(
    ...
    val isMutualFavorite = remember(peerID, peerNicknames) {
        try {
            ...
            if (peerID.length == 64 && peerID.matches(Regex("^[0-9a-fA-F]+$"))) {
                ...
                com.chriscorp.celer.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(noiseKeyBytes)?.isMutual == true
            } else if (peerID.length == 16 && peerID.matches(Regex("^[0-9a-fA-F]+$"))) {
                com.chriscorp.celer.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(peerID)?.isMutual == true
            } else false
    ...
    val titleText: String = if (isNostrDM) {
        ...
        val (conversationGeohash, baseName) = try {
            val repoField = com.chriscorp.celer.ui.GeohashViewModel::class.java.getDeclaredField("repo")
            repoField.isAccessible = true
            val repo = repoField.get(viewModel.geohashViewModel) as com.chriscorp.celer.nostr.GeohashRepository
            val gh = repo.getConversationGeohash(peerID) ?: "geohash"
            val fullPubkey = com.chriscorp.celer.nostr.GeohashAliasRegistry.get(peerID) ?: ""
        ...
        peerNicknames[peerID] ?: run {
            val titleFromFavorites = try {
                if (peerID.length == 64 && peerID.matches(Regex("^[0-9a-fA-F]+$"))) {
                    val noiseKeyBytes = peerID.chunked(2).map { it.toInt(16).toByte() }.toByteArray()
                    com.chriscorp.celer.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(noiseKeyBytes)?.peerNickname
                } else if (peerID.length == 16 && peerID.matches(Regex("^[0-9a-fA-F]+$"))) {
                    com.chriscorp.celer.favorites.FavoritesPersistenceService.shared.getFavoriteStatus(peerID)?.peerNickname
                } else null
...
private fun MainHeader(
    ...
    val bookmarksStore = remember { com.chriscorp.celer.geohash.GeohashBookmarksStore.getInstance(context) }
        ...
        Row(
            ...
            Row(verticalAlignment = Alignment.CenterVertically, modifier = Modifier.padding(end = 14.dp)) {
                ...
                val currentGeohash: String? = when (val sc = selectedLocationChannel) {
                    is com.chriscorp.celer.geohash.ChannelID.Location -> sc.channel.geohash
                    else -> null
                }
...
private fun LocationChannelsButton(
    ...
    val (badgeText, badgeColor) = when (selectedChannel) {
        ...
        is com.chriscorp.celer.geohash.ChannelID.Location -> {
            val geohash = (selectedChannel as com.chriscorp.celer.geohash.ChannelID.Location).channel.geohash
