...
import com.chriscorp.celer.nostr.GeohashMessageHandler
import com.chriscorp.celer.nostr.GeohashRepository
import com.chriscorp.celer.nostr.NostrDirectMessageHandler
import com.chriscorp.celer.nostr.NostrIdentityBridge
import com.chriscorp.celer.nostr.NostrProtocol
import com.chriscorp.celer.nostr.NostrRelayManager
import com.chriscorp.celer.nostr.NostrSubscriptionManager
...
class GeohashViewModel(
    ...
    private var locationChannelManager: com.chriscorp.celer.geohash.LocationChannelManager? = null
    ...
    fun initialize() {
        ...
        try {
            locationChannelManager = com.chriscorp.celer.geohash.LocationChannelManager.getInstance(getApplication())
            ...
            state.setSelectedLocationChannel(com.chriscorp.celer.geohash.ChannelID.Mesh)
    ...
    fun sendGeohashMessage(content: String, channel: com.chriscorp.celer.geohash.GeohashChannel, myPeerID: String, nickname: String?) {
        viewModelScope.launch {
            try {
                ...
                val localMsg = com.chriscorp.celer.model.BitchatMessage(
                    id = tempId,
                ...
                if (startedMining) {
                    com.chriscorp.celer.ui.PoWMiningTracker.startMiningMessage(tempId)
                }
                try {
                    ...
                    if (startedMining) {
                        com.chriscorp.celer.ui.PoWMiningTracker.stopMiningMessage(tempId)
                    }
                }
            } catch (e: Exception) {
    ...
    fun startGeohashDM(pubkeyHex: String, onStartPrivateChat: (String) -> Unit) {
        ...
        val gh = (current as? com.chriscorp.celer.geohash.ChannelID.Location)?.channel?.geohash
        if (!gh.isNullOrEmpty()) {
            repo.setConversationGeohash(convKey, gh)
            com.chriscorp.celer.nostr.GeohashConversationRegistry.set(convKey, gh)
        }
    ...
    fun blockUserInGeohash(targetNickname: String) {
        ...
        if (pubkey != null) {
            ...
            val sysMsg = com.chriscorp.celer.model.BitchatMessage(
                sender = "system",
    ...
    private fun switchLocationChannel(channel: com.chriscorp.celer.geohash.ChannelID?) {
        ...
        when (channel) {
            is com.chriscorp.celer.geohash.ChannelID.Mesh -> {
            ...
            is com.chriscorp.celer.geohash.ChannelID.Location -> {
                ...
                viewModelScope.launch {
                    ...
                    com.chriscorp.celer.nostr.GeohashAliasRegistry.put("nostr_${dmIdentity.publicKeyHex.take(16)}", dmIdentity.publicKeyHex)
                }
            }
            null -> {
